...
class TimeQueryModel:
    """Time-based query storage and management for manual time queries."""
    
    @classmethod
    def _ensure_table_exists(cls):
        """Ensure the time_queries table exists."""
        try:
            query = """
                CREATE TABLE IF NOT EXISTS time_queries (
                    ...
                    focus_areas TEXT, -- JSON string for focus areas
                    output_format TEXT DEFAULT 'summary',
                    status TEXT DEFAULT 'pending', -- pending, processing, completed, failed
                    result_content TEXT,
                    result_metadata TEXT, -- JSON string for additional metadata
                    created_timestamp DATETIME DEFAULT CURRENT_TIMESTAMP,
                    completed_timestamp DATETIME,
                    execution_time_ms INTEGER,
                    is_saved BOOLEAN DEFAULT 0, -- User-saved queries
                    query_name TEXT -- Name for saved queries
                )
            """
            db.execute_update(query)
            
            # Create indexes for performance
            db.execute_update("CREATE INDEX IF NOT EXISTS idx_time_queries_created ON time_queries(created_timestamp)")
            db.execute_update("CREATE INDEX IF NOT EXISTS idx_time_queries_time_range ON time_queries(time_range_start, time_range_end)")
            db.execute_update("CREATE INDEX IF NOT EXISTS idx_time_queries_saved ON time_queries(is_saved)")
            
            logger.info("TimeQueryModel table and indexes created successfully")
        except Exception as e:
            logger.error(f"Failed to create time_queries table: {e}")
    
    @classmethod
    def create_query(cls, query_text: str, time_range_start: datetime, time_range_end: datetime,
                    query_type: str = 'manual', focus_areas: List[str] = None,
                    output_format: str = 'summary', query_name: str = None) -> Optional[int]:
        """Create a new time-based query."""
        try:
            ...
            query = """
                INSERT INTO time_queries 
                (query_text, time_range_start, time_range_end, query_type, 
                 focus_areas, output_format, query_name)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """
            
            result = db.execute_update(query, (
                query_text, time_range_start, time_range_end, query_type,
                focus_areas_json, output_format, query_name
            ))
            ...
            logger.error(f"Failed to create time query: {e}")
            return None
    
    @classmethod
    def update_query_status(cls, query_id: int, status: str, result_content: str = None,
                           result_metadata: Dict[str, Any] = None, execution_time_ms: int = None) -> bool:
        """Update query status and results."""
        try:
            metadata_json = json.dumps(result_metadata) if result_metadata else None
            completed_timestamp = datetime.now() if status == 'completed' else None
            
            query = """
                UPDATE time_queries 
                SET status = ?, result_content = ?, result_metadata = ?, 
                    completed_timestamp = ?, execution_time_ms = ?
                WHERE id = ?
            """
            
            result = db.execute_update(query, (
                status, result_content, metadata_json, completed_timestamp,
                execution_time_ms, query_id
            ))
            
            return result > 0
            
        except Exception as e:
            logger.error(f"Failed to update time query {query_id}: {e}")
            return False
    
    @classmethod
    def get_query(cls, query_id: int) -> Optional[Dict[str, Any]]:
        """Get a specific time query by ID."""
        try:
            cls._ensure_table_exists()
            
            query = "SELECT * FROM time_queries WHERE id = ?"
            rows = db.execute_query(query, (query_id,))
            
            if rows:
                time_query = dict(rows[0])
                # Parse JSON fields
                time_query['focus_areas'] = json.loads(time_query['focus_areas']) if time_query['focus_areas'] else []
                time_query['result_metadata'] = json.loads(time_query['result_metadata']) if time_query['result_metadata'] else {}
                return time_query
            
            return None
            
        except Exception as e:
            logger.error(f"Failed to get time query {query_id}: {e}")
            return None
    
    @classmethod
    def get_recent_queries(cls, limit: int = 20) -> List[Dict[str, Any]]:
        """Get recent time queries."""
        try:
            cls._ensure_table_exists()
            
            query = """
                SELECT * FROM time_queries 
            ...
            logger.error(f"Failed to get recent time queries: {e}")
            return []
    
    @classmethod
    def get_saved_queries(cls) -> List[Dict[str, Any]]:
        """Get user-saved time queries."""
        try:
            cls._ensure_table_exists()
            ...
            logger.error(f"Failed to get saved time queries: {e}")
            return []
    
    @classmethod
    def save_query(cls, query_id: int, query_name: str) -> bool:
        """Mark a query as saved with a name."""
        try:
            query = "UPDATE time_queries SET is_saved = 1, query_name = ? WHERE id = ?"
            result = db.execute_update(query, (query_name, query_id))
            return result > 0
            
        except Exception as e:
            logger.error(f"Failed to save time query {query_id}: {e}")
            return False
    
    @classmethod
    def delete_query(cls, query_id: int) -> bool:
        """Delete a time query."""
        try:
            result = db.execute_update("DELETE FROM time_queries WHERE id = ?", (query_id,))
            return result > 0
        except Exception as e:
            logger.error(f"Failed to delete time query {query_id}: {e}")
            return False
